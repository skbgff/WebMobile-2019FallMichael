...
public class EmployerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.deleteEmployerBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                deleteFromDB();
            }
...
//        String ID = cursor.getString(cursor.getColumnIndexOrThrow(SampleDBContract.Employer._ID));
        //database.delete(SampleDBContract.Employer.TABLE_NAME,selection , selectionArgs);

        if(cursor.getCount()>0){

            cursor.moveToNext();// Move to next item in the list
            Long Employer_id = cursor.getLong(cursor.getColumnIndex(SampleDBContract.Employer._ID));

            db.delete(SampleDBContract.Employer.TABLE_NAME,SampleDBContract.Employer._ID + "=?", new String[]{Employer_id.toString()}); //new String[]{ID}
        }
